...
const ChatList: React.FC = () => {
  ...
  const selectChat = (otherUserId: string, chatName: string) => {
  ...
  return (
    <div className="bg-neutral-100 relative flex justify-center items-center min-h-screen overflow-y-hidden">
      ...
      <div className={`${selectedChat ? 'bg-neutral-100': ''} w-5/6 right-0 absolute`}>
        {selectedChat ?
        <Chat otherUserId={selectedChat} api={api} token={token} userId={userId} /> :
        null
        }
      </div>
    </div>
...
const Chat = ({ otherUserId, api, token, userId }: {otherUserId: string, api: AxiosInstance, token: string | null, userId: string | null}) => {
  ...
  const groupedMessages = groupBy(messages, (message: any) => 
  ...
  useEffect(() => {
    const url = (process.env.REACT_APP_SOCKET_URL) || (process.env.REACT_DEV_URL)
    if (url) {
