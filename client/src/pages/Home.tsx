...
import { fetchGenresAndBooks, Book } from "../slice/dashboard/dashboardSlice";


const Home: React.FC = () => {
    const books = useSelector((state: RootState) => state.dashboard.books);
    const [search, setSearch] = useState("");
    const dispatch = useDispatch<AppDispatch>();
    const { userId, isLoggedIn } = useSelector((state: RootState) => state.auth);
    const { selectedGenre } = useSelector((state: RootState) => state.dashboard);
