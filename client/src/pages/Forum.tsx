...
import { Post, createUserPost, deleteUserPost, fetchUserPosts, setPostText, setSearch, setShowCreateForum, setUpdatePost, updateUserPost } from '../slice/forum/forumSlice';


const Forum: React.FC = () => {
    const { isLoggedIn, userId } = useSelector((state: RootState) => state.auth);
    const dispatch = useDispatch<AppDispatch>();
    const { posts, search, updatePost, postText, status, showCreateForum } = useSelector((state: RootState) => state.forum);
    ...
    useEffect(() => {
        ...
        const observed = document.querySelector('.list-masonry')
        if (observed) {
    ...
    const handlePostSubmit = async (e: React.FormEvent) => {
        ...
        if (isLoggedIn) {
            try {
                if (updatePost) {
                    dispatch(updateUserPost({ postId: updatePost._id, postText}))
                } else {
                    dispatch(createUserPost({ postText, userId }))
                }
    ...
    return (
        <div className="bg-neutral-100  min-h-screen lg:pt-8 pt-3 flex flex-col">
            ...
            <div className="flex justify-center mb-5">
                <input type="text" className="outline-teal-500 focus:caret-teal-600 bg-teal-200 lg:w-96 w-64 lg:h-10 h-8 p-2 lg:text-base text-sm rounded-xl text-teal-800 placeholder:text-teal-800 hover:shadow-lg hover:shadow-teal-300" placeholder="looking for a specific user..." onChange={(e) => dispatch(setSearch(e.target.value))} />
            </div>
                ...
                <div className='sticky lg:inset-y-12 inset-y-4 mx-auto w-fit lg:p-5 pt-6 rounded z-40'>
                    ...
                    <form className="flex flex-col items-center justify-center lg:mb-5 mb-2" onSubmit={handlePostSubmit}>
                        <label className='flex flex-col items-center justify-center mb-2.5'>
                            <textarea className="outline-teal-500 focus:caret-teal-600 bg-teal-200 text-teal-950 placeholder:text-teal-800' lg:w-96 w-80 lg:h-36 h-24 p-2 lg:text-base text-sm rounded-xl resize-y hover:shadow-lg hover:shadow-teal-300 opacity-80" placeholder='Description...' value={postText} onChange={(e) => dispatch(setPostText(e.target.value))} required />
                        </label>
